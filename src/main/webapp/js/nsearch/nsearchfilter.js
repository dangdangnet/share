    ...
    var m = {
        ...
        isMore:function(){
            ...
            box.each(function(){
                ...
                var len = div.children("span").length;
        ...
        brandOver:function(){
            var logoBrand = this.box.children(".brand_pic");
            if(m.ifhaslogo){
                ...
                a.bind("mouseover", function(){
                    ...
                    $(this).children("img").length >0 && $(this).children('font').css('visibility','hidden') && $(this).children("img").show();
                });
            }
        },
        /*展开收起(大)*/
        ...
        bbmore:function(obj){
            ...
            if(m.ifhaslogo){
                ...
                if($(obj).hasClass("btn_out")){
                    $(obj).removeClass("btn_out").addClass("btn_in").html("收起").show();                       right.addClass("img_list_content_open");
                }else{
...
$(search).bind({
            ...
            box.children(".btn_no").bind("click", function(){
                ...
                if(r.children(".brand_box").length){
                    if($(this).parents(".list_right").children(".list_content").children("div").attr("id") ==1 && m.ifhaslogo){
                        li.height("auto");
                        m.cpicback(li.find(".list_right"),li.find(".list_right"));
                    }else{
        ...
        filterUrl : function( param ){
            ...
            if(url.indexOf('#J_tab') === -1) {
                var arr = url.split('?'), str = [];
            }else{
                var tmp = url.split('#J_tab');
                var lrr = tmp[0], urr = tmp[1];
                var arr = lrr.split('?'), str = [];
            }
            for (var i in arr) {
                if(i == 'indexOf') continue;
                if(i == 0) {
                    str.push( arr[i]+'?' );         // 域名和path部分
                }else{
                    var brr = arr[i].split('&');    //参数部分
                    var flag = 0;
                    for (var j in brr) {
                        if(j == 'indexOf') continue;
                        if(brr[j].indexOf('att=') === -1) {
                            str.push('&'+brr[j]);
                        } else {
                            var crr = brr[j].split('=');
                            if(crr[1]) {
